...
import { ChartItem, Scales } from './types';
declare type Orient = 'top' | 'bottom' | 'left' | 'right';
interface Data {
...
export default class AxisAnnotation implements ChartItem {
    ...
    resize: () => void;
    update: (data: Partial<Pick<Data, 'value' | 'text' | 'color'>>) => void;
    draw: () => void;
}
export {};
/**
 ...
 * /
module.exports = function(d3_svg_axis,
   d3_scale_linear, accessor_value, plot, plotMixin) {  // Injected dependencies
  return function() { // Closure function
    ...
    annotation.refresh = function(g) {
      var fmt = format ? format : (axis.tickFormat()
      ? axis.tickFormat() : axis.scale().tickFormat());
      refresh(p.dataSelector.select(g), p.accessor,
      axis, orient, fmt, height, width, point, translate);
    };
...
function refresh(selection, accessor, axis, orient, format, height, width, point, translate) {
  ...
  selection.select('text').text(textValue(accessor,
    format)).call(textAttributes, accessor, axis, orient, neg);
}
...
*/
//# sourceMappingURL=AxisAnnotation.d.ts.map